// 🎓 API ROUTES - MÓDULO SECRETÁRIO ESCOLAR (Corrigido)
...
const authMiddleware = async (req: AuthenticatedRequest, res: Response, next: any) => {
  try {
...
router.get('/people', authMiddleware, async (req: AuthenticatedRequest, res: Response) => {
  try {
    const {
      search,
...
router.get('/people/:id', authMiddleware, async (req: AuthenticatedRequest, res: Response) => {
  try {
    const { id } = req.params
...
router.post('/people', 
  ...
  validateInput(['cpf', 'name', 'birth_date', 'gender']),
...
// Listar escolas
router.get('/schools', authMiddleware, async (req: AuthenticatedRequest, res: Response) => {
  try {
...
router.get('/enrollments', authMiddleware, async (req: AuthenticatedRequest, res: Response) => {
  try {
    const {
      school_id,
      class_id,
  ...
  async (req: AuthenticatedRequest, res: Response) => {
    try {
      ...
      const dashboard = {
        ...
        average_frequency: stats[2].data ? 
          stats[2].data.reduce((sum: number, item: any) => 
            sum + item.frequency_percentage, 0) / (stats[2].data.length || 1) : 0,
